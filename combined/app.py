            ...
            if request.form['button'] == 'file':  
                ...
                if file and allowed_file(file.filename):
                    ...
                    if evidence_name == 'info':
                        ...
                        engine1 = create_engine('mysql://root:Sharon0108@localhost', echo=True)
...
def result(query,evidence, threshold):
    ...
    if request.method == 'GET':
        ...
        for row in datadf.itertuples(index = True, name = 'Pandas'):
            nodes.extend([getattr(row,'Name')])
            
       
        conn.close()
        if len(data) >0:
        # no url change now
            return render_template('result.html',tables=[datadf.to_html(classes='table')], titles = titles,querynode=querynode, nodes=nodes)
        else:
