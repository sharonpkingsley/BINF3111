	...
	if request.method == 'POST':
	   try:
				...
				if request.form['threshold'] == '':
					print "no threshold input";
				else:
					threshold_parameter = request.form['threshold']
					print threshold_parameter;

				return redirect(url_for('result', query=_query, evidence = chosen_evis, threshold = threshold_parameter))
	   except Exception as e:
...
def result(query,evidence,threshold):
	...
	if request.method == 'GET':
		...
		for evi in evidences:
			print evi
			args = (query, evi,threshold)
			#cursor.callproc('searchOneEvidence', args)
			#however, leave the no evidence as ''
			cursor.callproc('withThreshold', args)
